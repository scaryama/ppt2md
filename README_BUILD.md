# PPT to Markdown GUI - EXE 빌드 가이드
...
- `--name="PPT2Markdown"`: 생성될 exe 파일 이름
- `--windowed`: 콘솔 창 없이 GUI만 표시
- `--onefile`: 단일 exe 파일로 생성 (폴더 대신)
- `--add-data "ppt2md.py;."`: ppt2md.py 모듈을 포함
...
```
dist\PPT2Markdown.exe
```

## 배포

`dist\PPT2Markdown.exe` 파일만 복사하여 다른 Windows 컴퓨터에서 실행할 수 있습니다.
...
### python-pptx 모듈 오류
```bash
pyinstaller --hidden-import=pptx --hidden-import=pptx.shapes --hidden-import=pptx.shapes.table ...
```

### lxml 모듈 오류
python-pptx는 lxml에 의존합니다. 다음을 추가하세요:
```bash
pyinstaller --hidden-import=lxml --hidden-import=lxml.etree ...
```

### 파일 크기가 큰 경우
`--onefile` 옵션을 제거하면 여러 파일로 생성되지만 크기가 줄어듭니다:
```bash
pyinstaller --windowed --name="PPT2Markdown" ppt2md_gui.py
```
...
- python-pptx는 lxml에 의존하므로 lxml 관련 모듈도 포함해야 합니다
