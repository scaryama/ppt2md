"""
GUI application for converting PPT/PPTX files to Markdown format
...
from ppt2md import ppt_to_markdown
...
class ConversionThread(QThread):
    ...
    def __init__(self, ppt_path, output_dir=None):
        super().__init__()
...
class DropArea(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.label = QLabel("Drag and drop PPT/PPTX file here")
    ...
    def dragEnterEvent(self, event: QDragEnterEvent):
        ...
        if event.mimeData().hasUrls():
            # Only allow PPT/PPTX files
            urls = event.mimeData().urls()
            if urls and any(url.toLocalFile().lower().endswith(('.ppt', '.pptx')) for url in urls):
    ...
    def dropEvent(self, event: QDropEvent):
        ...
        if event.mimeData().hasUrls():
            urls = event.mimeData().urls()
            ppt_files = [url.toLocalFile() for url in urls if url.toLocalFile().lower().endswith(('.ppt', '.pptx'))]
            
            if ppt_files:
                # Use only the first PPT/PPTX file
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        """Initialize UI"""
        self.setWindowTitle("PPT to Markdown Converter")
        ...
        # Title
        title_label = QLabel("PPT to Markdown Converter")
        ...
        # Initial message
        self.log_message("=" * 60)
        self.log_message("PPT to Markdown Converter")
        self.log_message("=" * 60)
        self.log_message("Drag and drop a PPT/PPTX file or click the 'Select File' button.\n")
    ...
    def start_conversion(self, ppt_path):
        """Start conversion"""
        ...
        self.conversion_thread = ConversionThread(ppt_path)
